...
namespace VehicleResale.Tests.UnitTests;
...
public class VehicleUnitTests
{
    ...
    public void Should_Register_Sale_Successfully()
    {
        ...
        vehicle.PaymentStatus.Should().Be(PaymentStatus.Pending);
        vehicle.SaleDate.Should().BeOnOrAfter(beforeSale);
    ...
    public void Should_Update_Payment_Status_To_Confirmed()
    {
        ...
        // Act
        vehicle.UpdatePaymentStatus(PaymentStatus.Confirmed);
    ...
    public void Should_Cancel_Sale_When_Payment_Cancelled()
    {
        ...
        // Act
        vehicle.UpdatePaymentStatus(PaymentStatus.Cancelled);
    ...
    /// <summary>
    /// Testa se o sistema impede a atualização do status de pagamento de um veículo não vendido
    /// </summary>
    [Trait("Category", "Unit")]
    [Trait("Feature", "PaymentStatus")]
    [Fact]
    public void Should_Not_Update_Payment_If_Not_Sold()
    {
        // Arrange
        var vehicle = new Vehicle("Hyundai", "HB20", 2022, "Branco", 65000.00m);

        // Act & Assert
        var action = () => vehicle.UpdatePaymentStatus(PaymentStatus.Confirmed);
        action.Should().Throw<InvalidOperationException>()
              .WithMessage("Veículo não está vendido");

        vehicle.PaymentStatus.Should().Be(PaymentStatus.Pending);
        vehicle.IsSold.Should().BeFalse();

        _logger.LogInformation($"✅ Validação funcionando: não permite atualizar pagamento de veículo não vendido");
    }
